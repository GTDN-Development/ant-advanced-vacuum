...
export function Widget(props: React.ComponentProps<"div">) {
  return (
    <div
      {...props}
      className={clsx(props.className, "relative size-[648px] aspect-square")}
    >
      <div className="group absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <TileLabel className="absolute top-1/2 left-1/2 text-center -translate-x-1/2 -translate-y-1/2">
          Vacuum <br /> Chamber
        </TileLabel>
        <Center />
      </div>
      ...
      <div className="group absolute top-0 right-0 translate-y-2.5">
        <TileImage
          ...
          title="Desorption"
          className="absolute bottom-12.5 right-5 w-33.5"
        />
        <TileLabel className="absolute bottom-5 right-5">
          Vaporisation
        </TileLabel>
        <RightTop />
      </div>
      <div className="group absolute bottom-0 right-0 -translate-y-2.5">
        <TileImage
      ...
      <div className="group absolute bottom-0 right-0 -translate-x-2.5">
        <TileImage
      ...
      <div className="group absolute left-0 bottom-0 -translate-y-2.5">
        <TileImage
      ...
      <div className="group absolute left-0 top-0 translate-y-2.5">
        <TileImage
...
function TileLabel(props: React.ComponentProps<"span">) {
  return (
    <span
      {...props}
      className={clsx(
        "text-lg font-bold group-hover:text-white pointer-events-none",
        props.className,
      )}
    >
      {props.children}
  ...
  return (
    <div {...props} className={clsx(props.className, "pointer-events-none")}>
      <img
        src={props.src}
        alt={props.title}
        className="object-contain w-full group-hover:hidden"
      />
      <img
        src={props.srcOnHover}
        alt={props.title}
        className="object-contain w-full hidden group-hover:block"
      />
    </div>
  );
}
